...
var withValidator = function withValidator(rules, messages) {
  var validatorState = _lodash2.default.map(_lodash2.default.keys(rules), function (form) {
    return {
      name: form,
      message: '',
      rules: _lodash2.default.map(rules[form].split('|'), function (rule) {
        if (rule.indexOf('min:') !== -1 || rule.indexOf('max:') !== -1) {
          var minMax = rule.split(':');
          return _lodash2.default.assign({}, _defineProperty({ type: minMax[0] }, minMax[0], minMax[1]));
        }

        return _lodash2.default.assign({}, { type: rule });
      })
    };
  });

  var requiredForm = _lodash2.default.filter(_lodash2.default.keys(rules), function (form) {
  ...
  var validatorMsg = (typeof messages === 'undefined' ? 'undefined' : _typeof(messages)) !== undefined ? _lodash2.default.assign({}, defaultMsg, messages) : _lodash2.default.assign({}, defaultMsg);
    ...
    return _temp = _class = function (_Component) {
      ...
      function Validator(props) {
        ...
        _this.validate = function (key, value) {
          ...
          var index = _lodash2.default.findIndex(form, function (data) {
        ...
        _this.validating = function (formRules, index, formValue) {
          ...
          _lodash2.default.forEach(formRules, function (rule) {
            switch (rule.type) {
              case 'required':
        ...
        _this.getErrorMessage = function (key) {
          ...
          if (_lodash2.default.filter(form, function (f) {
            return f.name === key;
          }).length > 0) {
            return _lodash2.default.filter(form, function (f) {
              return f.name === key;
            })[0].message;
          }
        ...
        _this.formIsValid = function () {
          ...
          var errCount = _lodash2.default.filter(form, function (f) {
            return f.message !== '';
          }).length;
          var complete = _lodash2.default.filter(_lodash2.default.keys(required), function (formIndex) {
